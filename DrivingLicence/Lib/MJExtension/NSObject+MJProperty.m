...
+ (NSString *)propertyKey:(NSString *)propertyName
{
    ...
    // 查看有没有需要替换的key
    if ([self respondsToSelector:@selector(faf_replacedKeyFromPropertyName121:)]) {
        key = [self faf_replacedKeyFromPropertyName121:propertyName];
    }
    // 兼容旧版本
    ...
    // 查看有没有需要替换的key
    if (!key && [self respondsToSelector:@selector(faf_replacedKeyFromPropertyName)]) {
        key = [self faf_replacedKeyFromPropertyName][propertyName];
    }
    // 兼容旧版本
...
+ (Class)propertyObjectClassInArray:(NSString *)propertyName
{
    ...
    if ([self respondsToSelector:@selector(faf_objectClassInArray)]) {
        clazz = [self faf_objectClassInArray][propertyName];
    }
    // 兼容旧版本
...
#pragma mark - --公共方法--
+ (void)faf_enumerateProperties:(MJPropertiesEnumeration)enumeration
{
    // 获得成员变量
...
+ (NSMutableArray *)properties
{
    ...
    if (cachedProperties == nil) {
        ...
        [self faf_enumerateClasses:^(__unsafe_unretained Class c, BOOL *stop) {
            // 1.获得所有的成员变量
...
#pragma mark - 新值配置
+ (void)faf_setupNewValueFromOldValue:(MJNewValueFromOldValue)newValueFormOldValue
{
    objc_setAssociatedObject(self, &MJNewValueFromOldValueKey, newValueFormOldValue, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

+ (id)faf_getNewValueFromObject:(__unsafe_unretained id)object oldValue:(__unsafe_unretained id)oldValue property:(MJProperty *__unsafe_unretained)property{
    // 如果有实现方法
    if ([object respondsToSelector:@selector(faf_newValueFromOldValue:property:)]) {
        return [object faf_newValueFromOldValue:oldValue property:property];
    }
    // 兼容旧版本
...
+ (void)faf_setupObjectClassInArray:(MJObjectClassInArray)objectClassInArray
{
    [self faf_setupBlockReturnValue:objectClassInArray key:&MJObjectClassInArrayKey];
    
    [[MJDictionaryCache dictWithDictId:&MJCachedPropertiesKey] removeAllObjects];
}

#pragma mark - key配置
+ (void)faf_setupReplacedKeyFromPropertyName:(MJReplacedKeyFromPropertyName)replacedKeyFromPropertyName
{
    [self faf_setupBlockReturnValue:replacedKeyFromPropertyName key:&MJReplacedKeyFromPropertyNameKey];
    
    [[MJDictionaryCache dictWithDictId:&MJCachedPropertiesKey] removeAllObjects];
}

+ (void)faf_setupReplacedKeyFromPropertyName121:(MJReplacedKeyFromPropertyName121)replacedKeyFromPropertyName121
{
...
{
    [self faf_enumerateProperties:enumeration];
}

+ (void)setupNewValueFromOldValue:(MJNewValueFromOldValue)newValueFormOldValue
{
    [self faf_setupNewValueFromOldValue:newValueFormOldValue];
}
...
{
    return [self faf_getNewValueFromObject:object oldValue:oldValue property:property];
}

+ (void)setupReplacedKeyFromPropertyName:(MJReplacedKeyFromPropertyName)replacedKeyFromPropertyName
{
    [self faf_setupReplacedKeyFromPropertyName:replacedKeyFromPropertyName];
}

+ (void)setupReplacedKeyFromPropertyName121:(MJReplacedKeyFromPropertyName121)replacedKeyFromPropertyName121
{
    [self faf_setupReplacedKeyFromPropertyName121:replacedKeyFromPropertyName121];
}

+ (void)setupObjectClassInArray:(MJObjectClassInArray)objectClassInArray
{
    [self faf_setupObjectClassInArray:objectClassInArray];
}
@end
